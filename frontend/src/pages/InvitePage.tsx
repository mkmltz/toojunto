...
export function InvitePage({
  ...
  return <AuthLayout><section className="card invite-received"><span className="badge">Convite</span><h1>Você foi convidado para participar de um Grupo</h1><dl><div><dt>Grupo</dt><dd>{convite.group_name}</dd></div><div><dt>Cota</dt><dd>{formatarValor(convite.quota_value)}</dd></div><div><dt>Participantes</dt><dd>{convite.participant_limit}</dd></div><div><dt>Vagas disponíveis</dt><dd>{convite.available_slots}</dd></div><div><dt>Início</dt><dd>{formatarData(convite.start_date)}</dd></div></dl>{erro && <p className="alert error" role="alert">{erro}</p>}<button className="btn btn-primary" type="button" disabled={aceitando} onClick={onEntrar}>{aceitando ? "Entrando..." : "Entrar no Grupo"}</button>{!autenticado && <button className="btn btn-secondary" type="button" disabled={aceitando} onClick={onCriarConta}>Criar minha conta</button>}<button className="btn btn-quiet" type="button" disabled={aceitando} onClick={onAgoraNao}>Agora não</button></section></AuthLayout>;
}
