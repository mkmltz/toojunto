...
const rotuloSituacao: Record<SituacaoCiclo, string> = { ATUAL: "Em andamento", PROXIMO: "Próximo", CONCLUIDO: "Concluído" };
...
export function GroupDetailsPage({ nomeUsuario, usuarioId, grupo, aviso, carregando, onVoltar, onEditar, onCancelar, onObterConvite, onPrepararSorteio, onRealizarSorteio }: GroupDetailsPageProps) {
  ...
  const pagamentosConfirmados = obrigacoes?.filter((item) => item.situacao === "CONFIRMADO").length ?? 0;
  ...
  return <AppShell nome={nomeUsuario}>
    ...
    {temCiclos && progresso && <><section className="card current-cycle" aria-labelledby="titulo-ciclo-atual"><h2 id="titulo-ciclo-atual">{grupoConcluido ? "Último ciclo" : "Ciclo atual"}</h2><div className="cycle-current"><strong>Ciclo {progresso.ciclo_atual} de {progresso.total_ciclos}</strong><p>Contemplado: <b>{progresso.contemplado_ciclo_atual}</b></p><p>Data prevista: <b>{formatarData(progresso.data_prevista_ciclo_atual)}</b></p>{obrigacoes && <p className="payment-progress"><b>{pagamentosConfirmados} de {obrigacoes.length}</b> pagamentos confirmados</p>}</div>{carregandoObrigacoes && <p role="status">Carregando pagamentos do ciclo...</p>}{erroObrigacoes && !obrigacaoSelecionada && <p className="alert error" role="alert">{erroObrigacoes}</p>}{obrigacoes && <section className="payments-section" aria-labelledby="titulo-pagamentos"><h3 id="titulo-pagamentos" ref={tituloPagamentos} tabIndex={-1}>{grupoConcluido ? "Pagamentos do último ciclo" : "Pagamentos do ciclo atual"}</h3><ul className="payments-list">{obrigacoes.map((item) => {
    ...
    })}</ul></section>}</section><section className="card upcoming-cycles" aria-labelledby="titulo-proximos"><h2 id="titulo-proximos">Próximos recebimentos</h2><ol className="cycle-list">{progresso.ciclos.map((ciclo) => <li key={ciclo.numero_ciclo} className={`cycle-item cycle-${ciclo.situacao.toLowerCase()}`}><div><strong>{ciclo.numero_ciclo}º ciclo</strong><span className="cycle-status">{rotuloSituacao[ciclo.situacao]}</span></div><p><b>{ciclo.nome}</b> · {ciclo.papel === "GESTOR" ? "Gestor" : "Participante"}</p><p>Data prevista: {formatarData(ciclo.data_prevista)}</p></li>)}</ol></section></>}
    ...
    {temCiclos && <section className="card group-info" aria-labelledby="titulo-informacoes"><h2 id="titulo-informacoes">Informações do grupo</h2><div className="info-disclosure"><button type="button" aria-expanded={regrasAbertas} aria-controls="regras-grupo" onClick={() => setRegrasAbertas((abertas) => !abertas)}>Regras do grupo</button><div id="regras-grupo" hidden={!regrasAbertas} className="group-summary"><dl><div><dt>Valor por ciclo</dt><dd>{formatarValor(grupo.valor_cota)}</dd></div><div><dt>Valor do prêmio</dt><dd>{formatarValor(grupo.valor_premio)}</dd></div><div><dt>Participantes</dt><dd>{grupo.quantidade_participantes}</dd></div><div><dt>Ciclos</dt><dd>{grupo.quantidade_ciclos}</dd></div><div><dt>Data de início</dt><dd>{formatarData(grupo.data_inicio)}</dd></div></dl></div></div><div className="info-disclosure"><button type="button" aria-expanded={participantesAbertos} aria-controls="participantes-grupo" onClick={() => setParticipantesAbertos((abertos) => !abertos)}>Participantes ({grupo.formacao.quantidade_atual})</button><div id="participantes-grupo" hidden={!participantesAbertos} className="group-members"><ul>{grupo.formacao.participantes.map((participante, indice) => <li key={`${participante.nome}-${indice}`}><span>{participante.nome}</span><small className={participante.papel === "GESTOR" ? "manager-role" : ""}>{participante.papel === "GESTOR" ? "Gestor" : "Participante"}</small></li>)}</ul><p className="formation-status">{mensagemVagas}</p></div></div></section>}
    {!temCiclos && <><section className="card group-summary" aria-label="Detalhes do grupo"><dl><div><dt>Valor por ciclo</dt><dd>{formatarValor(grupo.valor_cota)}</dd></div><div><dt>Valor do prêmio</dt><dd>{formatarValor(grupo.valor_premio)}</dd></div><div><dt>Participantes</dt><dd>{grupo.quantidade_participantes}</dd></div><div><dt>Ciclos</dt><dd>{grupo.quantidade_ciclos}</dd></div><div><dt>Data de início</dt><dd>{formatarData(grupo.data_inicio)}</dd></div></dl></section>
